...
exports.edit = async (req, res) => {
  ...
  if (!gallery.user.equals(jwt.id)) return res.json({ error: 'Authentication error' });
...
exports.add = async (req, res) => {
  ...
  const NFT = {
    id: req.body.id,
    order: gallery.nfts.length ? gallery.nfts[gallery.nfts.length - 1].order + 1 : 0,
...
exports.reorderGalleries = async (req, res) => {
  ...
  if (!req.body.galleries || !req.body.galleries.length) return res.json({ error: 'Issue reordering' });
...
exports.reorderCollection = async (req, res) => {
  ...
  if (!req.body.nfts || !req.body.nfts.length) return res.json({ error: 'Issue reordering' });
