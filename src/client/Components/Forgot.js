        ...
        { (!submitting && !submitted) &&
          <form onSubmit={ submit }>
            <div className='form__group field'>
              <input type='text' className='form__field' placeholder='Username or Email' name='username' id='username' required maxLength='100' onChange={ e => setData({ recovery: e.target.value }) } />
              <label className='form__label'>Username or Email</label>
            </div>
            <input type='submit' value='Send Recovery Email' className='submit-button' />
          </form>
        }
        { err &&
