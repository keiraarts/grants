...
export default function Collection({ editing, setEditCollection, galleries, addNewGallery, setGalleries, username }) {
  ...
  async function getMyAssets(offset, items) {
    await fetch(`https://api.opensea.io/api/v1/assets?order_direction=desc&offset=${ offset }&limit=50&owner=${ provider.selectedAddress}`, {
        method: 'GET',
        headers: {
          'Accept': 'application/json',
          'Content-Type': 'application/json',
  ...
  return (
    <div>
      ...
      { manage ?
        <div className='flex'>
          <div>
            <div className='text-l'><strong>{ manage.name }</strong></div>
            <div className='text-m'><strong>{ manage.description }</strong></div>
          </div>
          <div className='flex-full' />
          <span className='text-s text-grey pointer' onClick={ () => setManage(null) }>Back</span>
        </div>
      :
        <div className='flex'>
      ...
      <div className='margin-top-s'>
        { editing ?
          <div>
            { manage ?
              <div className='small-button margin-left-s' onClick={ () => setEditGallery(true) } >
                <div className='text-s'>Edit Name / Description</div>
              </div>
              :
              <div className='small-button margin-left-s' onClick={ () => setCreate(true) } >
      ...
      { editGallery &&
        <EditGallery manage={ manage } galleries={ galleries } setGalleries={ setGalleries } setEditGallery={ setEditGallery } />
      }
      { create &&
        <Create addNewGallery={ addNewGallery } setCreate={ setCreate } />
      }
      { (gallery && !editing) &&
        <div className='margin-top'>
          ...
          <div className='margin-top-s'>
            ...
            :
              <div className='margin-top-s'>There are no NFTs in this gallery yet</div>
            }
          </div>
        </div>
      }
      ...
      { manage &&
        <ManageGallery gallery={ manage } reorderNFTs={ reorderNFTs } galleries={ galleries } setGalleries={ setGalleries } />
      }
    </div>
  );
}
