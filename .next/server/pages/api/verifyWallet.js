...
/* harmony import */ var ethers__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(4879);
          ...
          case 6:
            try {
              _src_server_services_authorization_service__WEBPACK_IMPORTED_MODULE_2___default()(req.headers.authorization, res, function (jwt) {
                _models_userModel__WEBPACK_IMPORTED_MODULE_4__.default.findById(jwt.id, function (err, user) {
                  if (err) return res.json(err);
                  if (!user) return res.status(401).json({
                    err: "Authentication error"
                  });else {
                    var signedAddress = ethers__WEBPACK_IMPORTED_MODULE_5___default().utils.verifyMessage("Verify wallet address for Sevens Foundation", req.body.signature);
...
var __webpack_exports__ = (__webpack_exec__(9323));
