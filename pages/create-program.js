...
export async function getStaticProps() {
  const res = await fetch(`${apiUrl()}/program/getMyOrgs`, {
    method: "POST",
    headers: {
...
export default function Application(props) {
  ...
  return (
    <div className="content-block">
      <div className="text-l text-b">Create a Grant Program</div>
      <div className="margin-top">
        <div className="ethos-text">
          Sevens Foundation is purely focused on artistic and social integrity.
          Participating exhibitions must have either a community upbringing,
        ...
        <div className="margin-top text-s">
          <em>Starred fields are optional*</em>
        </div>
        <form onSubmit={submit}>
          <div className="margin-top">Program Curator or Organization</div>
          {org ? (
            <div>
              <Link className="text-rainbow" to={`/curator/${org.url}`}>
                {org.name}
              </Link>
            </div>
          ) : (
            <div>
              ...
              <div className="text-s">@{data.twitter}</div>
              <div className="form__group field">
                <input
                  type="text"
                  className="form__field"
              ...
              <div className="text-s">@{data.instagram}</div>
            </div>
          )}
          <div className="margin-top">Program Details</div>
          <div className="form__group field">
            <input
              type="text"
              className="form__field"
          ...
          <div className="form__group field">
            <input
              ...
              maxLength="100"
              onChange={(e) =>
                setData({ ...data, url: doDashes(e.target.value) })
              }
            />
          ...
          {submitted && (
            <div>
              <div className="margin-top text-s text-rainbow">
                Thank you for submitting your program! Please let us know when
